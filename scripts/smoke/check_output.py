#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""x264guiEx 終了バッチ用: 出力ファイル / ログ / aup2 設定のスモークチェック。"""
...
def extract_log_x264_options(log_text: str) -> str | None:
    lines = log_text.splitlines()
    for idx, line in enumerate(lines):
        if "x264 options" in line:
            buf: list[str] = []
            for fol in lines[idx + 1 :]:
                ...
                if not s:
                    ...
                    continue
                # 次セクションっぽい行で打ち切り
                if re.match(r"^(AAC|raw |x264 \[|L-SMASH|CPU|encoded |converting |Aviutl)", s):
                    break
                if s.startswith("---"):
                    break
                buf.append(s.strip())
...
def compare_cli(expected: str, actual: str) -> list[str]:
    ...
    if not expected:
        return problems
    if not actual:
        problems.append("ログから x264 options を抽出できませんでした")
        return problems
...
def main() -> int:
    ap = argparse.ArgumentParser(description="x264guiEx smoke check (after-bat)")
    ...
    name_keys = cfg.get("plugin_name_contains") or ["x264guiEx"]
    ...
    if not logpath.is_file():
        ...
        log_cli = extract_log_x264_options(log_text)
        if log_cli:
            ok(f"ログ CLI 抽出: {log_cli[:120]}{'...' if len(log_cli) > 120 else ''}")
        else:
            ng("ログから x264 options を抽出できませんでした")
        ...
        if aup2 and aup2.is_file():
            try:
                ...
                if conf is None:
                    ng(f"aup2 から x264guiEx 設定を見つけられません: {aup2}")
                else:
