...
<#
.SYNOPSIS
  x264guiEx をビルドし、AviUtl2 Plugin へ配置して起動する。
...
$Sln = Join-Path $RepoRoot "x264guiEx.sln"
...
if ($Platform -eq "x64") {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\x64\x264guiEx.auo2"
} else {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\Win32\x264guiEx.auo"
}
...
$dstAuo2 = Join-Path $PluginDir "x264guiEx.auo2"
Write-Host "==> copy"
Write-Host "    $srcAuo2"
Write-Host " -> $dstAuo2"
...
foreach ($name in @("x264guiEx.ini", "x264guiEx.en.ini", "x264guiEx.zh.ini")) {
